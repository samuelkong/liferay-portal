...
<c:if test="<%= !viewMode.equals(Constants.PRINT) %>">
	<div class="export-action user-tool-asset-addon-entry">
		...
		<liferay-ui:icon
			...
			label="<%= true %>"
			message='<%= LanguageUtil.format(resourceBundle, "x-download-x-as-x", new Object[] {"hide-accessible", articleDisplay.getTitle(), StringUtil.toUpperCase(HtmlUtil.escape(extension))}) %>'
			method="get"
