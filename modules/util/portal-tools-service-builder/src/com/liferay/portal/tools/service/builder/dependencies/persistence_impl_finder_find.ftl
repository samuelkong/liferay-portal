				...
				<#if finder.hasColumn("groupId") && finder.getColumn("groupId").hasArrayableOperator()>
					...
					<#list finderColsList as finderCol>
						<#if finderCol.name == "groupId">
							...
							${finderCol.type}[] isNotEnabledGroupIds = new ${finderCol.type}[0];
							${finderCol.type}[] isEnabledGroupIds = ${finderCol.names};

							for (${finderCol.type} ${finderCol.name} : ${finderCol.names}) {
								if (!InlineSQLHelperUtil.isEnabled(${finderCol.name})) {
									isNotEnabledGroupIds = ArrayUtil.append(isNotEnabledGroupIds, ${finderCol.name});

									isEnabledGroupIds = ArrayUtil.remove(isEnabledGroupIds, ${finderCol.name});
								}
							}

							if (isEnabledGroupIds.length == 0) {
								${finderCol.names} = isNotEnabledGroupIds;
							...
							${finderCol.names} = isEnabledGroupIds;
