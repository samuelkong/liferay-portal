...
public class OrderByComparatorFactoryImpl implements OrderByComparatorFactory {
	...
	public OrderByComparator create(
		...
		try {
			tableName = (String)clazz.getDeclaredField("TABLE_NAME").get(null);
			modelName = clazz.newInstance().getModelClass().getSimpleName();
		} catch (Exception e) {
		...
		if (Validator.isNotNull(orderByCol) &&
			...
			portalPreferences.setValue(
				portletKey, modelKey + orderByTypeKey, orderByType);
		}
		else {
			for (int i = 0; i < defaultOrderByColumns.length; i += 2) {
				if (
						String.valueOf(defaultOrderByColumns[i+1]).equals(
						Constants.ASC)) {
