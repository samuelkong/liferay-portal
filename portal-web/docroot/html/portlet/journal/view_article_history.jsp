		...
		<aui:script>
			<c:if test="<%= JournalArticlePermission.contains(permissionChecker, article, ActionKeys.DELETE) %>">
				Liferay.provide(
					...
					function() {
						var deleteArticles = true;

						var deleteArticleIds = Liferay.Util.listCheckedExcept(document.<portlet:namespace />fm, "<portlet:namespace />allRowIds");

						if (!deleteArticleIds) {
							deleteArticles = false;
						}
						else if (!confirm('<%= UnicodeLanguageUtil.get(pageContext, "are-you-sure-you-want-to-delete-the-selected-version") %>')) {
							deleteArticles = false;
						}
						...
						if (deleteArticles) {
							...
							document.<portlet:namespace />fm.<portlet:namespace />articleIds.value = deleteArticleIds;
			...
			<c:if test="<%= JournalArticlePermission.contains(permissionChecker, article, ActionKeys.EXPIRE) %>">
				Liferay.provide(
					...
					function() {
						var expireArticles = true;

						var expireArticleIds = Liferay.Util.listCheckedExcept(document.<portlet:namespace />fm, "<portlet:namespace />allRowIds");

						if (!expireArticleIds) {
							expireArticles = false;
						}
						else if (!confirm('<%= UnicodeLanguageUtil.get(pageContext, "are-you-sure-you-want-to-expire-the-selected-version") %>')) {
							expireArticles = false;
						}
